...
public class MonthlyCalendar {
	...
	public static void main(String[] args) {
		...
		System.out.print("出力したい年を数字で入力してください: ");
		int targetYear = scn.nextInt();

		Calendar calendar = Calendar.getInstance();
		calendar.set(Calendar.YEAR, targetYear);

		int definedYear = calendar.get(Calendar.YEAR);

		for (int month = 1; month <= 12; month++) {
			System.out.println(definedYear + "年" + month + "月");
			System.out.println(" 日" + " 月" + " 火" + " 水" + " 木" + " 金" + " 土");
			System.out.println(new MonthlyCalendar(definedYear, month));
		}
		scn.close();
	}
}